			...
			<?php
				...
				foreach ($_POST as $cle => $element){
					...
					if ($ind != ""){//Si le nom de la matière a été renseigné
						...
						UpdateNote($value, $data['CODE_DISCIPLINE'], $_POST['reference']);
						$indiceEffectue=1;
						mysql_free_result($req);
					}//if ($ind != "")
				}//foreach ($_POST as $cle => $element)
				
				if ($indiceEffectue==1){//C'est que l'update s'est fait
					echo '<h4 align="center"><font color="green">Les Notes ont été bien modifiées!<br />Pour vérifier rendez vous sur 
